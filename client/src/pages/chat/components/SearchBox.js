...
const SearchBox = () => {
    ...
    const handleOnChanged = (ev) => {
        ...
        if (ev.target.value !== "") {
            ...
            fetch(
                `${process.env.REACT_APP_API_URL}/user/?search=${ev.target.value}`,
                {
                    method: "GET",
                    headers: {
    ...
    const handleKeyDown = (ev) => {
        switch (ev.key) {
            case "Enter": {
                if (isListShown()) handleSelect(data[selectedSuggestionIndex]);
                break;
            }
            case "ArrowUp": {
    ...
    const renderSuggestions = () =>
        data.map((suggestion, index) => {
            const { _id, userName, image } = suggestion;
            const jonctionIndex =
                userName.toLowerCase().indexOf(value.toLowerCase()) +
                value.length;
