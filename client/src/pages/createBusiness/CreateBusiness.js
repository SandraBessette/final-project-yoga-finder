...
const CreateBusiness = ({ type }) => {
    ...
    const handleChangeApp = (ev) => {
        setFormData({
            ...formData,
            address: { ...formData.address, app: ev.target.value },
        });
    };
    ...
    const handleHoursChange = (ev, day, item) => {
        setFormData({
            ...
            hours: {
                ...formData.hours,
                [day]: { ...formData.hours[day], [item]: ev.target.value },
            },
        });
    };
    ...
    const handleOnClick = (ev) => {
        ...
        const confirmationLink =
            type === "New" ? "/user/new/confirmation" : "/user/confirmation";
        fetch(endpoint, {
            method: method,
            headers: {
    ...
    const handleTags = (ev, value) => {
        ...
        if (ev.target.checked) tagsArray.push(value);
        else {
    ...
    const panTo = useCallback(
        ({ lat, lng, bounds, formatAddress, zipCode }) => {
            setFormData({
                ...formData,
                address: { formatted: formatAddress, zip: zipCode },
    ...
    const formZipValidation = useCallback(() => {
        ...
        if (formData.address.zip === "") return false;
        ...
        if (
            ...
            setValidZipError(
                "The zip code is not a valid zip code in Canada. Try to choose a more precise location in Canada"
            );
            isValid = false;
        } else setValidZipError("");
    ...
    const formPhoneValidation = useCallback(() => {
        ...
        if (
            ...
            setValidPhoneError(
                "The phone number should have a 999-999-9999 format"
            );
            isValid = false;
        } else setValidPhoneError("");
    ...
    return (
        <Wrapper>
            <UserHeader
                title={type === "New" ? "Create Business" : "Modify Business"}
            />
            <MainWrapper>
                ...
                {status === "idle" && (
                    <Form>
                        <Image
                            src={formData.image[0] || "/noYogaImage.jpg"}
                            alt="yogaPicture"
                        ></Image>
                        <TextBoxWrapper>
                            ...
                            {Object.values(filters.filter).map((tag) => {
                                return (
                                    <Checkbox
                                        ...
                                        isChecked={formData.tags.includes(
                                            tag.name
                                        )}
                                    >
                                        {tag.name}
