...
const SideBar = ({ data, status }) => {
    ...
    const handleChangeFilter = useCallback((ev, value, index) => {
        setTempFilter((prev) => ({
            ...prev,
            [index]: { name: value, isChoosen: ev.target.checked },
        }));
        // dispatch(updataAnimatedId(id));
    }, []);
        ...
        <Wrapper>
                ...
                {Object.keys(filters.type).map((filterItem) => {
                    return (
                        <TypeButton
                            key={filterItem}
                            className={
                                filters.type[filterItem] ? "selected" : null
                            }
                            type={filterItem}
                            margin="0 5px 5px 0"
                            onclick={(e) =>
                                handleTypeButtonClick(e, filterItem)
                            }
                        />
                    );
                })}
            ...
            {filterShow ? (
                <FilterWrapper>
                    ...
                    <ButtonWrapper>
                        <Button width={"100px"} onclick={handleCancelClick}>
                            Cancel
                        </Button>
                        <Button width={"100px"} onclick={handleDoneClick}>
                            Apply
                        </Button>
                    </ButtonWrapper>
                </FilterWrapper>
                        ...
                        <>
                            {data && data.length === 0 && (
                                <NoResultWrapper>
                                    ...
                                    <p>
                                        To see more results, try panning or
                                        zooming the map{" "}
                                    </p>
                                    <VscSearchStop
                                        color="lightgray"
